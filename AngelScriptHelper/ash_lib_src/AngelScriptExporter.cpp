...
using AngelScript::asITypeInfo;
using AngelScript::asIScriptFunction;
using AngelScript::asEBehaviours;
using AngelScript::asIStringFactory;
using AngelScript::asIScriptModule;
#endif
...
namespace AngelScriptExporter {
	...
	std::string FormatType(int typeID, asIScriptEngine* engine, asIScriptModule* module) {
		...
		if (typeID & AngelScript::asTYPEID_APPOBJECT && engine) {
		...
		if (type) {
			...
			if (typeID & AngelScript::asTYPEID_HANDLETOCONST) {
				name = "const " + name;
			}
			return name;
		}
	...
	std::string FormatBehaviour(AngelScript::asEBehaviours b) {
		std::string ret;
		switch (b) {
	...
	Value FormatModuleFunction(asIScriptFunction* asFunc, asIScriptEngine* engine, AngelScript::asIScriptModule* module, MemoryPoolAllocator<>& allocator) {
		Value func = Value(kObjectType);
		func.AddMember("Name", Value(asFunc->GetName(), allocator), allocator);
		func.AddMember("Namespace", Value(asFunc->GetNamespace(), allocator), allocator);
		Value params = Value(kArrayType);
	...
	void ExportEngineAsJSON(const char* file, asIScriptEngine* engine) {
		...
		for (uint32_t i = 0; i < typeCount; ++i) {
			...
			type.AddMember("Flags", Value((uint32_t)asType->GetFlags() & AngelScript::asOBJ_MASK_VALID_FLAGS), output.GetAllocator());
			...
			for (uint32_t k = 0; k < behaviourCount; ++k) {
				...
				if (asFunc) {
					...
					if (behaviour == AngelScript::asBEHAVE_CONSTRUCT) {
						decl = std::string("void ") + decl;
					} else if (behaviour == AngelScript::asBEHAVE_DESTRUCT) {
						decl.erase(0, 1);
						decl = std::string("void ") + decl;
					}
			...
			for (uint32_t k = 0; k < factoryCount; ++k) {
				...
				if (asFunc) {
					Value func = FormatEngineFunction(asFunc, engine, output.GetAllocator());
					Value behave = Value(kObjectType);
					behave.AddMember("Type", Value(AngelScript::asBEHAVE_FACTORY), output.GetAllocator());
					std::string decl = func["Declaration"].GetString();
	...
	public:
		StringFactory() {}
		const void* GetStringConstant(const char* data, AngelScript::asUINT length) {
		...
		int GetRawStringData(const void* str, char* data, AngelScript::asUINT* length) const {
			if (str == 0)
				return AngelScript::asERROR;

			if (length)
				*length = (AngelScript::asUINT)reinterpret_cast<const std::string*>(str)->length();

			if (data)
				memcpy(data, reinterpret_cast<const std::string*>(str)->c_str(), reinterpret_cast<const std::string*>(str)->length());
			return AngelScript::asSUCCESS;
		}
	private:
	...
	static bool ScriptArrayTemplateCallback(asITypeInfo* ti, bool& dontGarbageCollect) {
		...
		if (typeId == AngelScript::asTYPEID_VOID)
			return false;
		if ((typeId & AngelScript::asTYPEID_MASK_OBJECT) && !(typeId & AngelScript::asTYPEID_OBJHANDLE)) {
			asITypeInfo* subtype = ti->GetEngine()->GetTypeInfoById(typeId);
			AngelScript::asDWORD flags = subtype->GetFlags();
			if ((flags & AngelScript::asOBJ_VALUE) && !(flags & AngelScript::asOBJ_POD)) {
				// Verify that there is a default constructor
				bool found = false;
				for (AngelScript::asUINT n = 0; n < subtype->GetBehaviourCount(); n++) {
					asEBehaviours beh;
					asIScriptFunction* func = subtype->GetBehaviourByIndex(n, &beh);
					if (beh != AngelScript::asBEHAVE_CONSTRUCT) continue;
				...
				if (!found) {
					...
					ti->GetEngine()->WriteMessage("array", 0, 0, AngelScript::asMSGTYPE_ERROR, "The subtype has no default constructor");
					return false;
				}
			} else if ((flags & AngelScript::asOBJ_REF)) {
				bool found = false;
				...
				if (!ti->GetEngine()->GetEngineProperty(AngelScript::asEP_DISALLOW_VALUE_ASSIGN_FOR_REF_TYPE)) {
					// Verify that there is a default factory
					for (AngelScript::asUINT n = 0; n < subtype->GetFactoryCount(); n++) {
				...
				if (!found) {
					// No default factory
					// TODO: Should format the message to give the name of the subtype for better understanding
					ti->GetEngine()->WriteMessage("array", 0, 0, AngelScript::asMSGTYPE_ERROR, "The subtype has no default factory");
					return false;
				}
			}

			// If the object type is not garbage collected then the array also doesn't need to be
			if (!(flags & AngelScript::asOBJ_GC))
				dontGarbageCollect = true;
		} else if (!(typeId & AngelScript::asTYPEID_OBJHANDLE)) {
			// Arrays with primitives cannot form circular references,
			// thus there is no need to garbage collect them
			dontGarbageCollect = true;
		} else {
			assert(typeId & AngelScript::asTYPEID_OBJHANDLE);
			...
			if (!(flags & AngelScript::asOBJ_GC)) {
				if ((flags & AngelScript::asOBJ_SCRIPT_OBJECT)) {
					...
					if ((flags & AngelScript::asOBJ_NOINHERIT)) {
		...
		if (document.HasMember("GlobalTypes")) {
			...
			for (const auto& type : globalTypes.GetArray()) {
				...
				if (flags & AngelScript::asOBJ_SCRIPT_OBJECT) {
					...
					r = engine->RegisterObjectType(decl, (flags & AngelScript::asOBJ_VALUE) ? 1 : 0, flags);
				}
				assert(r >= 0);
				for (auto& beh : type["Behaviors"].GetArray()) {
					asEBehaviours b = (asEBehaviours)beh["Type"].GetInt();
					if (b == AngelScript::asBEHAVE_TEMPLATE_CALLBACK) {
						r = engine->RegisterObjectBehaviour(decl, b, beh["Func"]["Declaration"].GetString(), AngelScript::asFUNCTION(ScriptArrayTemplateCallback), AngelScript::asCALL_CDECL);
					} else {
						r = engine->RegisterObjectBehaviour(decl, b, beh["Func"]["Declaration"].GetString(), AngelScript::asFUNCTION(0), AngelScript::asCALL_GENERIC);
					}
					assert(r >= 0);
				}
			}
				...
				if (asType) {
					...
					for (auto& method : type["Methods"].GetArray()) {
						if (flags & AngelScript::asOBJ_SCRIPT_OBJECT) {
							...
							r = engine->RegisterObjectMethod(decl, methodDecl, AngelScript::asFUNCTION(0), AngelScript::asCALL_GENERIC);
							assert(r >= 0);
						}
					}
				}
	...
	void ExportModuleAsJSON(const char* file, AngelScript::asIScriptModule* module, asIScriptEngine* engine) {
		Document output;
		output.SetObject();
		...
		for (uint32_t i = 0; i < varCount; ++i) {
			...
			int typeID;
			bool isConst = false;
			int r = module->GetGlobalVar(i, &name, &nameSpace,&typeID, &isConst);
